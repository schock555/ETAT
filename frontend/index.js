  ...
  if (!(globalConfig.get('etsyOathToken') && globalConfig.get('etsyOathSecret'))) { // If there's no linked Etsy Account
    ...
    content = (
      <Box display="flex" flexDirection="column" alignItems="center" padding={4}>
        ...
        <Text fontStyle="italic" padding={2}>{'testThis block lets you create (draft) Etsy listings directly from Airtable'}</Text>
...
function AuthBlock() {
  ...
  async function submitVerifier() {
    ...
    const shippingProfileResponse = await fetch(requestData.url, {
      method: 'GET',
      mode: 'cors',
      cache: 'no-cache',
      headers: {
    ...
    const shippingProfiles = await shippingProfileResponse.json();
    let shippingProfileID = shippingProfiles.results[0].shipping_profile_id;

    // Add shipping template ID to use for the createListing calls later
    if (globalConfig.hasPermissionToSet('shippingProfileID', shippingProfileID)) {
      await globalConfig.setAsync('shippingProfileID', shippingProfileID);
    }
